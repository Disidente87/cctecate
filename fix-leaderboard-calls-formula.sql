-- =====================================================
-- FIX: Cambiar fórmula del leaderboard para usar available_percentage de llamadas
-- En lugar de progress_percentage (llamadas a tiempo), usar available_percentage (llamadas completadas)
-- =====================================================

-- Actualizar la función get_leaderboard_data para usar available_percentage
...
BEGIN
  ...
  IF user_role = 'admin' THEN
    ...
    SELECT 
      ...
      ls.calls_score,
      (ls.goals_completion_percentage * 0.5 
       + ls.activities_completion_percentage * 0.25 
       + ls.calls_score * 0.25)::DECIMAL(5,2) as total_score,
      ROW_NUMBER() OVER (
        ORDER BY (ls.goals_completion_percentage * 0.5 
                + ls.activities_completion_percentage * 0.25
                + ls.calls_score * 0.25) DESC
